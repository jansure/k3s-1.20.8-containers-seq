...
func (m *kubeGenericRuntimeManager) SyncPod(pod *v1.Pod, podStatus *kubecontainer.PodStatus, pullSecrets []v1.Secret, backOff *flowcontrol.Backoff) (result kubecontainer.PodSyncResult) {
	...
	start := func(typeName string, spec *startSpec) error {
		...
		if len(pod.Spec.Containers) > 1 {
			// 当前要启动的容器的名称等于循环找到的容器名称,并且是容器序列中的最后一个容器
			if spec.container.Name == pod.Spec.Containers[len(pod.Spec.Containers) - 1].Name {
				preContainerName := pod.Spec.Containers[len(pod.Spec.Containers) - 2].Name
				containerStatus, flag := getContainerStatus(pod.Status.ContainerStatuses, preContainerName)
				if !flag {
					msg := "the last container failed"
					utilruntime.HandleError(fmt.Errorf("%v %+v judged last index container starting failed in pod %v: %v: %s", typeName, spec.container, format.Pod(pod), err, msg))
					startContainerResult.Fail(err, msg)
					return  fmt.Errorf("%v %+v judged last index container starting " +
						"failed in pod %v: %v: %s", typeName, spec.container, format.Pod(pod), err, msg)
				}
				if !containerStatus.Ready {
					msg := "the last container is not ready"
					utilruntime.HandleError(fmt.Errorf("%v %+v judged last index container starting failed in pod %v: %v: %s", typeName, spec.container, format.Pod(pod), err, msg))
					startContainerResult.Fail(err, msg)
					return  fmt.Errorf("%v %+v judged last index container starting " +
						"failed in pod %v: %v: %s", typeName, spec.container, format.Pod(pod), err, msg)
